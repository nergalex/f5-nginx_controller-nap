...
class AzureRMVirtualMachineScaleSet(AzureRMModuleBase):
    ...
    def __init__(self):
        ...
        self.module_arg_spec = dict(
            ...
            subnet_mgt=dict(type='str'),
            subnet_dataplane=dict(type='str'),
            load_balancer_mgt=dict(type='str'),
            load_balancer_dataplane=dict(type='str'),
        ...
        self.os_type = None
        self.subnet_mgt = None
        self.subnet_dataplane = None
        self.virtual_network_resource_group = None
        self.virtual_network_name = None
        self.tags = None
        self.differences = None
        self.load_balancer_mgt = None
        self.load_balancer_dataplane = None
        ...
        mutually_exclusive = [('load_balancer_dataplane', 'application_gateway')]
        self.results = dict(
            changed=False,
            actions=[],
    ...
    def exec_module(self, **kwargs):
        ...
        subnet_mgt = None
        subnet_dataplane = None
        image_reference = None
        load_balancer_mgt_backend_address_pools = None
        load_balancer_mgt_inbound_nat_pools = None
        load_balancer_mgt = None
        load_balancer_dataplane_backend_address_pools = None
        load_balancer_dataplane_inbound_nat_pools = None
        load_balancer_dataplane = None
        ...
        if self.state == 'present':
            ...
            if self.load_balancer_mgt:
                load_balancer_mgt = self.get_load_balancer(self.load_balancer_mgt)
                load_balancer_mgt_backend_address_pools = ([self.compute_models.SubResource(id=resource.id)
                                                        for resource in load_balancer_mgt.backend_address_pools]
                                                       if load_balancer_mgt.backend_address_pools else None)
                load_balancer_mgt_inbound_nat_pools = ([self.compute_models.SubResource(id=resource.id)
                                                    for resource in load_balancer_mgt.inbound_nat_pools]
                                                   if load_balancer_mgt.inbound_nat_pools else None)

            if self.load_balancer_dataplane:
                load_balancer_dataplane = self.get_load_balancer(self.load_balancer_dataplane)
                load_balancer_dataplane_backend_address_pools = ([self.compute_models.SubResource(id=resource.id)
                                                        for resource in load_balancer_dataplane.backend_address_pools]
                                                       if load_balancer_dataplane.backend_address_pools else None)
                load_balancer_dataplane_inbound_nat_pools = ([self.compute_models.SubResource(id=resource.id)
                                                    for resource in load_balancer_dataplane.inbound_nat_pools]
                                                   if load_balancer_dataplane.inbound_nat_pools else None)
        ...
        try:
            ...
            if self.state == 'present':
                ...
                for cur_index in (0,1):
                    backend_address_pool = nicConfigs[cur_index]['properties']['ipConfigurations'][0]['properties'].get('loadBalancerBackendAddressPools', [])
                    backend_address_pool += nicConfigs[cur_index]['properties']['ipConfigurations'][0]['properties'].get('applicationGatewayBackendAddressPools', [])
                    lb_or_ag_id = None
                    if len(nicConfigs) != 1 or len(backend_address_pool) != 1:
                        ...
                        if cur_index == 0 and load_balancer_mgt:
                            lb_or_ag_id = "{0}/".format(load_balancer_mgt.id)
                        elif cur_index == 1 and load_balancer_dataplane:
                            lb_or_ag_id = "{0}/".format(load_balancer_dataplane.id)
                        elif application_gateway:
                            lb_or_ag_id = "{0}/".format(application_gateway.id)

                        backend_address_pool_id = backend_address_pool[cur_index].get('id')
                        if cur_index == 0 \
                                and lb_or_ag_id is not None \
                                and (bool(lb_or_ag_id) != bool(backend_address_pool_id) or not backend_address_pool_id.startswith(lb_or_ag_id)):
                            differences.append('load_balancer_mgt')
                            changed = True
                        if cur_index == 1 \
                                and lb_or_ag_id is not None \
                                and (bool(lb_or_ag_id) != bool(backend_address_pool_id) or not backend_address_pool_id.startswith(lb_or_ag_id)):
                            differences.append('load_balancer_dataplane')
                            changed = True
        ...
        if changed:
            if self.state == 'present':
                if not vmss:
                    ...
                    if self.subnet_mgt:
                        subnet_mgt = self.get_subnet(self.virtual_network_name, self.subnet_mgt)

                    if self.subnet_dataplane:
                        subnet_dataplane = self.get_subnet(self.virtual_network_name, self.subnet_dataplane)
                        ...
                        virtual_machine_profile=self.compute_models.VirtualMachineScaleSetVMProfile(
                            ...
                            network_profile=self.compute_models.VirtualMachineScaleSetNetworkProfile(
                                network_interface_configurations=[
                                    self.compute_models.VirtualMachineScaleSetNetworkConfiguration(
                                        ...
                                        ip_configurations=[
                                            self.compute_models.VirtualMachineScaleSetIPConfiguration(
                                                ...
                                                primary=True,
                                                load_balancer_backend_address_pools=load_balancer_mgt_backend_address_pools,
                                                load_balancer_inbound_nat_pools=load_balancer_mgt_inbound_nat_pools,
                                            )
                                        ],
                                    ...
                                    self.compute_models.VirtualMachineScaleSetNetworkConfiguration(
                                        name=self.name + '-dataplane',
                                        ip_configurations=[
                                            self.compute_models.VirtualMachineScaleSetIPConfiguration(
                                                name='default',
                                                subnet=self.compute_models.ApiEntityReference(
                                                    id=subnet_dataplane.id
                                                ),
                                                primary=True,
                                                load_balancer_backend_address_pools=load_balancer_dataplane_backend_address_pools,
                                                load_balancer_inbound_nat_pools=load_balancer_dataplane_inbound_nat_pools,
                    ...
                    if support_lb_change:
                        if self.load_balancer_mgt:
                            vmss_resource.virtual_machine_profile.network_profile.network_interface_configurations[0] \
                                .ip_configurations[0].load_balancer_backend_address_pools = load_balancer_mgt_backend_address_pools
                            vmss_resource.virtual_machine_profile.network_profile.network_interface_configurations[0] \
                                .ip_configurations[0].load_balancer_inbound_nat_pools = load_balancer_mgt_inbound_nat_pools
                            vmss_resource.virtual_machine_profile.network_profile.network_interface_configurations[0] \
                                .ip_configurations[0].application_gateway_backend_address_pools = None
                        elif self.load_balancer_dataplane:
                            vmss_resource.virtual_machine_profile.network_profile.network_interface_configurations[1] \
                                .ip_configurations[0].load_balancer_backend_address_pools = load_balancer_dataplane_backend_address_pools
                            vmss_resource.virtual_machine_profile.network_profile.network_interface_configurations[1] \
                                .ip_configurations[0].load_balancer_inbound_nat_pools = load_balancer_dataplane_inbound_nat_pools
                            vmss_resource.virtual_machine_profile.network_profile.network_interface_configurations[1] \
                                .ip_configurations[0].application_gateway_backend_address_pools = None
